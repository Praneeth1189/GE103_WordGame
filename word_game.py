#this is an inbuilt library for GUI programming in Python
from tkinter import *
...
def selecting_a_word(word_list,i):
    '''This function select a word from a word list of 3000 word in random manner
    but with specific world length and return it'''
    flag=0
    #lets iterate into word_list using while loop
    while(flag!=1):
        #select a random word
        x =random.randint(1,len(word_list))
        #if word length equal to i then found a required word length
        if(len(word_list[x])==i):
            #to stop while loop
            flag=1
    return word_list[x]
...
label4=Label(root, text='''       Welcome! To Word Game.
        ...
        1.In this flavor, you can length of word everytime.
        2.In this flavor, you get lengthier/tougher words level by level.
        So, please choose a flavor or exit the game.''')
